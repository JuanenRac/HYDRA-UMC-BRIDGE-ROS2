...
- **[HYDRA-UMC-MQTT-BROKER](https://github.com/JuanenRac/HYDRA-UMC-MQTT-BROKER)** — el transporte real de `mqtt_transport.py` para los propios topics `hydra/bridges/ros2/...` de este puente (la puerta de trabajo solo-de-plan, una llamada real de parada segura `std_srvs/Trigger`, y el topic de estado ROS 2 real republicado como el "adaptador desplegado por separado" que `rclpy_transport.py` siempre anticipó) - ver el propio `docs/BRIDGE_TOPICS.md` de ese repositorio.
